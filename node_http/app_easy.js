...
// Creo array de nombre de peliculas para luego ordenarlas alfabeticamente
// Creo arrau promedio de peliculas para luego obtener el promedio de las peliculas con mas de 7
let nombre_peliculas = [];
let promedio_peliculas = 0;

// Creo array nombre de peliculas ordenadas alfabeticamente
let nombre_peliculas_order = [];

// Agrego nombre de peliculas al array nombre_peliculas
movies.forEach(function(element){
	nombre_peliculas.push(element.original_title)
});

// Ordeno alfabeticamente el array nombre_peliculas y se lo pasa a nombre_peliculas_order
nombre_peliculas_order = nombre_peliculas.sort();

// creo objeto con las peliculas más votadas, es decir
// aquellas con más de 7
let most_voted = movies.filter(element => element.vote_average >= 7);

// Calculo el promedio del total de las peliculas más votadas
...
http.createServer((req, res) => {
	...
	switch (req.url) {
		...
		case '/':
			...
			res.write("\n");
			nombre_peliculas_order.forEach(element => res.write(`Pelicula: ${element} \n`))
			for (let i = 0; i < 5;i++){
				res.write("\n");
			}
		...
		case '/en-cartelera':
			...
			res.write("\n");
			res.write("Total Peliculas en Cartelera: " + movies.length + "\n");
			res.write("\n");
			movies.forEach(function(element){
		...
		case '/mas-votadas':
			...
			res.write("\n");
			res.write("Peliculas más votadas: " + most_voted.length + "\n");
			res.write("\n")
			res.write("Promedio: " + promedio_peliculas + "\n");
			res.write("\n")
			most_voted.forEach(function(element){
				res.write("Pelicula: " + element.title +"\n");
			...
			break;
		case '/sucursales':
			res.write("Nuestars Salas\n");
			res.write("\n");
			res.write("Tenemos " + theaters.length + "salas\n");
			res.write("\n");
			theaters.forEach(element => {res.write("Sobre nuestras salas: " + element.description + "\n")
			res.write("\n")});
			res.end('');
			// res.end('Sucursales');
			break;
		...
		case '/preguntas-frecuentes':
			...
			res.write("\n");
			res.write("Total de FAQS: " + faqs.length + "\n");
			res.write("\n");
			faqs.forEach(element => {res.write("Pregunta: " + element.faq_title + "\n")
